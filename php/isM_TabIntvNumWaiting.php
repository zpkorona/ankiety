<?php
//isM_TabIntvNumWaiting.php?user_id=xyz14&int_no=014&stage_no=1
//ALL ARGUMENTS MUST BE SET
//RETURNS int_no IF IT'S waiting____________ FOR GIVEN user_id AND stage_no
...
if (array_key_exists('survey_id', $_GET) &&
  ...
  $file_name   = "../$survey_id/data/intvnums_m_tab.txt";
        ...
        if (array_key_exists('intv_num', $params) && array_key_exists('user_id', $params)) {
          ...
          if ($req_intv_num === $intv_num) {
            ...
            if ($req_user_id === $user_id && array_key_exists($statusN, $params)) {
              $req_intv_num_match = trim($params[$statusN]) == $waitingstr;
            }//if
          }//if
        }//if
      }//while
