...
function getSurveysInfoFromXML () {
  ...
  xhr.open("GET", "_xml_/badanie.xml", false);//SYNCHRONICZNIE
  xhr.send();
  if (xhr.status != 200) {
...
function assignIntvNum (tstUserId, tstIntvNum, tstStageNo) {
  window.console.log("assignIntvNum\n" + "intvNumAuto=" + intvNumAuto + ", intvNumTable=" + intvNumTable + ", tstUserId=" + tstUserId + ", tstIntvNum=" + tstIntvNum);
  if (intvNumAuto || intvNumTable && (tstIntvNum != -1 || !intvNumShow)) {//JEŻELI IDENTYFIKATOR NADAWANY AUTOMATYCZNIE LUB Z TABELI
...
//=====================================================================================
//ZMIANA - POCZĄTEK BLOKU ZMIAN 1/6 Example.tst
//-------------------------------------------------------------------------------------
      //TEXT/VALUE
      case "tp1_ord":      //ZMIANA
...
//-------------------------------------------------------------------------------------
//ZMIANA - KONIEC BLOKU ZMIAN 1/6 Example.tst
//=====================================================================================
    }//switch
  }//for od ciasteczek
...
//=====================================================================================
//ZMIANA - POCZĄTEK BLOKU ZMIAN 2/6 Example.tst
//-------------------------------------------------------------------------------------
        //TEXT/VALUE
        case "tp1_ord":      //ZMIANA
        case "tp1_name":     //ZMIANA
...
//-------------------------------------------------------------------------------------
//ZMIANA - KONIEC BLOKU ZMIAN 2/6 Example.tst
//=====================================================================================
      }//switch
    }//for
...
//=====================================================================================
//ZMIANA - POCZĄTEK BLOKU ZMIAN 3/6 Example.tst
//-------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------
//ZMIANA - KONIEC BLOKU ZMIAN 3/6 Example.tst
//=====================================================================================
  startDataIsSet = true;
}//saveStartData
...
function gotoNextQuestion () {
  window.console.log("gotoNQ::" + currQuest + "::");//->" + questsTab[ordQsTab[currQuest]][0] + ":" + questsTab[ordQsTab[currQuest]][1]);
...
function gotoPrevQuestion () {
  window.console.log("gotoPrevQuestion: " + currQuest + "->" + questsTab[ordQsTab[currQuest]][0] + ":" + questsTab[ordQsTab[currQuest]][1]);
  secondLength = 0;
  if (1 < currQuest) {
    document.getElementById(questsTab[ordQsTab[currQuest]][0]).style.display = "none";
    currQuest--;
...
function gotoFirstEmptyQuestion () {
  window.console.log("gotoFE::" + currQuest + "::");//->" + questsTab[ordQsTab[currQuest]][0] + ":" + questsTab[ordQsTab[currQuest]][1]);
  if (questsTab[ordQsTab[currQuest]][3](questsTab[ordQsTab[currQuest]][0], questsTab[ordQsTab[currQuest]][1])) {//funkcja sprawdzająca przeszła
    if (currQuest < ordQsTab.length - 1) {//jeśli nie jest to ostanie pytanie
      currQuest++;
      questsTab[ordQsTab[currQuest]][2](questsTab[ordQsTab[currQuest]][0], questsTab[ordQsTab[currQuest]][1]);
...
//=====================================================================================
//ZMIANA - POCZĄTEK BLOKU ZMIAN 5/6 Example.tst
//-------------------------------------------------------------------------------------
//OBSŁUGA ANKIETY =====================================================================
...
function arrangeOrdQsTab () {
  ...
  var scenario = (intvNum - 1) % scenariaNum;  //WYBÓR WIERSZA W TABELI scenariaTab -- intvNum,//powinno być jak jest dla każdego
  var questsNum = scenariaTab[0].length;
  var firstItem = Math.floor((intvNum - 1) / scenariaNum) % questsNum; //pierwszy w rotacji wybranego wiersza tabeli scenariaTab
  var firstQSIndex = 1000;
  var i;
  window.console.log("arrangeOrdQsTab: [" + intvNum + "] (" + scenariaNum + "," + questsNum + ")");
  for (i = 0; i < questsNum; i++) {
    if (scenariaTab[scenario][i] < firstQSIndex) {
      firstQSIndex = scenariaTab[scenario][i];
    } //if
  }//for
/*
  for (i = 0; i < questsNum - firstItem; i++) {
    ordQsTab[firstQSIndex + i] = scenariaTab[scenario][i + firstItem];
  }//for
  for (; i < questsNum; i++) {
    ordQsTab[firstQSIndex + i] = scenariaTab[scenario][i + firstItem - questsNum];
  }//for
*/
  //wersja alternatywna i dosyć uproszczona
  for (i = 0; i < questsNum; i++) {
    ordQsTab[firstQSIndex + i] = scenariaTab[scenario][i];
    ordQsTab[firstQSIndex + i + 5] = scenariaTab[scenario][i] + 5;
  }//for

  window.console.log(ordQsTab);
}//arrangeOrdQsTab
...
function make_q8_arrLine () {
  var arrTabLen = q8_arrTab.length;
  var arrTabKey = (intvNum - 1) % arrTabLen + 1;//intvNum,//powinno być jak jest dla każdego
  var arrTabRow = 0;                               //index wiersza w tabeli q8_arrTab
  var orgTabLen = q8_orgTab.length;
  var firstItem = Math.floor((intvNum - 1) / arrTabLen) % orgTabLen; //pierwszy w rotacji wybranego wiersza tabeli q8_arrTab
  var i;                                        //WYBÓR WIERSZA w tabeli q8_arrTab
  window.console.log("make_q8_arrLine\n arrTabKey=" + arrTabKey + ", firstItem=" + (firstItem+1) + ":");
  for (i = 0; i < arrTabLen && q8_arrTab[i][0] != arrTabKey; i++);//SZUKAJ WIERSZA WEDŁUG arrTabKey - może być intvNum
  if (i < arrTabLen && q8_arrTab[i][0] == arrTabKey) {             //jeśli odnaleziony
    arrTabRow = i;
  }//if
  for (i = 1; i <= orgTabLen - firstItem; i++) {//ZAPISYWANIE q8_arrLine UWZGLĘDNIAJĄC OBRACANIE WYBRANEGO WIERSZA
    q8_arrLine[i] = q8_arrTab[arrTabRow][i + firstItem];
  }//for
  for (; i <= orgTabLen; i++) {//OBRÓĆ DLA WYBRANEGO WIERSZA autokręcioła jeżeli ma być
    q8_arrLine[i] = q8_arrTab[arrTabRow][i + firstItem - orgTabLen];
  }//for
  //for (i = 1; i <= orgTabLen; i++) window.console.log(q8_arrLine[i] + ",");
  window.console.log(q8_arrLine);
}//make_q8_arrLine

function arrange_q8_items () {
  var orgTabLen = q8_orgTab.length;
  var i;
  window.console.log("arrange_q8_items");
  make_q8_arrLine();
  for (i = 0; i < orgTabLen; i++) {
    document.getElementById("q8_item" + q8_orgTab[i][0]).innerHTML = q8_orgTab[q8_arrLine[i + 1] - 1][2];
//    document.getElementById("tp2_q3item" + q8_orgTab[i][0]).innerHTML = q8_orgTab[q8_arrLine[i + 1] - 1][2];
  }//for
}//arrange_q8_items

function rearrange_q8_data__ (q8_) {
  var orgTabLen = q8_orgTab.length;
  var q8_name;
  var i, j;
window.console.log("rearrange_q8_data__(" + q8_ +")");
  for (i = 0; i < orgTabLen; i++) {    //PRZEPISANIE danych z formularza do tabeli q8_orgTab
    q8_name = q8_ + q8_orgTab[i][0];
    for (j = 0; j < document.questForm[q8_name].length; j++) {
      if (document.questForm[q8_name][j].checked) {
        q8_orgTab[q8_arrLine[i + 1] - 1][1] = j;
      }//if
    }//for
  }//for
  for (i = 0; i < orgTabLen; i++) {    //PRZEPISANIE danych z q8_orgTab do formularza
    q8_name = q8_ + q8_orgTab[i][0];
    for (j = 0; j < document.questForm[q8_name].length; j++) {
      document.questForm[q8_name][j].checked = j == q8_orgTab[i][1];
    }//for
  }//for
}//rearrange_q8_data__

function rearranged_q8_index (find) {
  var orgTabLen = q8_orgTab.length;
  var i;
  for (i = 1; i <= orgTabLen && q8_arrLine[i] != find; i++);
  return i;
}//rearranged_q8_index

function rearrange_q8_data () {
  make_q8_arrLine();
  rearrange_q8_data__("q8_");
//  rearrange_q8_data__("tp2_q3_");
}//rearrange_q8_data
...
function arrangeQuestions () {
  ...
  arrange_q8_items(); //ZMIANA
  arrangeOrdQsTab();
  findRotation();
}//arrangeQuestions
//-------------------------------------------------------------------------------------
//ZMIANA - KONIEC BLOKU ZMIAN 5/6 Example.tst
//=====================================================================================
...
function verifyInt_num (qId, fldName) {
  ...
  var tstIntvNum;
  window.console.log("reset");
  window.console.log("verInum(" + qId + ":" + fldName + ")\nino=" + field.value + ", ronly=" + field.readOnly);
...
//=====================================================================================
//ZMIANA - POCZĄTEK BLOKU ZMIAN 6/6 Example.tst
//-------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------
//ZMIANA - KONIEC BLOKU ZMIAN 6/6 Example.tst
//=====================================================================================
