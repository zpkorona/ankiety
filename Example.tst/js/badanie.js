...
function assignIntvNum (tstIntvNum) {
  ...
  if (intvNumAuto ||                                        //JEŻELI IDENTYFIKATOR NADAWANY AUTOMATYCZNIE LUB
    ...
    if (tempFileExists(tstIntvNum) && isIntvNumStarted(userId, tstIntvNum, stageNum) && restoreFromTempFile() ||
        isIntvNumUsable(userId, tstIntvNum, stageNum)) {//sprawdza tstIntvNum >--> ustawia intvNum
    ...
    if (tstIntvNum == -1) {//nie ma numeru
      ...
      if (isIntvNumWaiting(userId, tstIntvNum, stageNum)) {
        document.questForm.intv_num.readOnly = true;
      } else {
        intvNum = -1;
        intvNumShow = true;
...
function make_q8_arrLine () {
  let arrTabLen = q8_arrTab.length;
  let arrTabKey = (intvNum - 1) % arrTabLen + 1;//intvNum,//powinno być jak jest dla każdego
  let arrTabRow = 0;                               //index wiersza w tabeli q8_arrTab
  let orgTabLen = q8_orgTab.length;
  let firstItem = Math.floor((intvNum - 1) / arrTabLen) % orgTabLen; //pierwszy w rotacji wybranego wiersza tabeli q8_arrTab
  let i;                                        //WYBÓR WIERSZA w tabeli q8_arrTab
...
function verifyInt_num (qId, fldName) {
  ...
  if (field.readOnly) {
    isOk = restoredIntv || useIntvNum(userId, field.value, stageNum);
  } else {
    ...
    if (tstIntvNum && intvNumIsInRange(tstIntvNum)) {
      if (intvNumGiven) {
        if (!useIntvNum(userId, tstIntvNum, stageNum)) {
          if (isIntvNumStarted(userId, tstIntvNum, stageNum) &&
        ...
        getIntvNum(userId, tstIntvNum, stageNum);
      }//else
...
function intvNumIsInRange (iNum, withRotations, ...ranges) {
  window.console.log("intvNumIsInRange(" + iNum + ", " + withRotations + ", " + ranges + ")");
  let isOk = false;
  let i;
...
//  document.getElementById("intv-num-info").innerHTML = parIntvNum == -1 ? "" : parIntvNum;
  ...
  if (cookiesTab.length < 2) {//JEŚLI OKAZUJE SIĘ ŻE NIE MA ŻADNYCH CIASTECZEK TO WYŚWIETLIĆ INFORAMCJĘ
    ...
    if (ckSurveyId != -1 && ckSurveyId == surveyId &&
      ...
      if (isIntvNumStarted(ckUserId, ckIntvNum, ckStageNum)) {//I WYWIAD JEST started
